        ...
        <div className="flex items-end h-[690px] md:h-screen px-[20px] pb-[20px] md:px-[40px] md:pb-[40px]" style={{backgroundImage: `url(${url})`}}>
            <div className="flex flex-col md:flex-row gap-1">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="font-primary text-white text-[24px] font-light hidden md:block">{description}</div>
                </div>
                <div className="flex flex-row flex-wrap justify-start md:justify-end gap-1">
                    {items.map((item, index) => (
                        <div key={index}>
                            <div className="border border-white rounded-full px-[16px] py-[8px] font-primary font-extralight text-white">{item}</div>
                        </div>
                    ))}
                </div>
            </div>
